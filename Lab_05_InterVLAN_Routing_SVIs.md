...
**Objective:**  
Configure inter-VLAN routing using a **multilayer switch (MLS)** and **Switch Virtual Interfaces (SVIs)** instead of router-on-a-stick.
...
| MLS1 | Routing Active | show ip route | Connected routes |
| All Switches | Trunks | show interfaces trunk | VLANs 10,20 allowed |
| SW1/SW2 | PC Ports | show interfaces fa0/2 switchport | Access mode correct VLAN |
...
1. How is this design different from router-on-a-stick?  
2. What role do SVIs play in this network?  
3. Why must we enable `ip routing` on the multilayer switch?  
4. What happens if VLAN 10 is missing on SW1?
...
| **MLS** | Switch that can route |
| **Inter-VLAN Routing** | Done via SVIs |
| **Verification** | `show ip int brief`, `show ip route`, `show interfaces trunk` |
